...
static int rtp_read(struct rtp_context_t* ctx, socket_t s)
{
	...
	if (r < 12)
		return -1;
    struct sockaddr_in *tests = ((struct sockaddr_in*)&ss);
    char *p=inet_ntoa(tests->sin_addr);
...
//    assert(0 == memcmp(&((struct sockaddr_in*)&ss)->sin_addr, &((struct sockaddr_in*)&ctx->ss)->sin_addr, 4));
    ...
    printf("\n");
    for (int i=0; i<19; i++) {
        printf("%02x ",(unsigned char)(ctx->rtp_buffer[i]));
        if (i == 11) {
            printf("\n");
        }
    }
...
static void rtp_packet2(void* param, const void *packet, int bytes, uint32_t time, int flags)
{
    const uint8_t start_code[] = { 0, 0, 0, 1 };
    struct rtp_context_t* ctx;
    ctx = (struct rtp_context_t*)param;
    ...
    unsigned char buf[2048*50] = {'\0'};
    memcpy(buf, packet, bytes);
    fwrite(packet, 1, bytes, ctx->fp);
    
}
static struct rtp_context_t* ctx = NULL;

int rtp_read2(struct rtp_context_t* ctx, socket_t s)
{
...
int rtp_receiver2(struct rtp_context_t* ctx, socket_t rtp[2], int timeout)
{
    if (ctx == NULL) {
        
        //        struct rtp_context_t* ctx = (struct rtp_context_t*)pp;
        ...
        handler.alloc = NULL;
        handler.free = NULL;
        handler.packet = rtp_packet2;
        
        ctx = malloc(sizeof(struct rtp_context_t));
        ctx->payload = rtp_payload_decode_create(payload, encoding, &handler, ctx);
        if (NULL == ctx->payload)
            return NULL; // ignore
        snprintf(ctx->rtp_buffer, sizeof(ctx->rtp_buffer), "%s.%s", "111", encoding);
        ctx->fp = fopen(ctx->rtp_buffer, "wb");
        strcpy(ctx->encoding, encoding);
    }

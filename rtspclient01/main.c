...
//  Copyright © 2019 ireader. All rights reserved.
//
...
//    snprintf(packet, sizeof(packet), "rtsp://%s/%s", host, file); // url
    snprintf(packet, sizeof(packet), "rtsp://%s/", host); // url
...
void rtsp_client_test2(const char* host, const char* file)
{
    int r;
    struct rtsp_client_test_t ctx;
    struct rtsp_client_handler_t handler;
    ...
    assert(0 == rtp_socket_create(NULL, ctx.rtp[0], ctx.port[0]));
    printf("port:%d port2:%d\n",ctx.port[0][0],ctx.port[0][1]);
    
    rtp_receiver2(NULL, ctx.rtp[0], 2000);
}

int main(int argc, const char * argv[]) {
    // insert code here...
    printf("Hello, World!\n");
    rtsp_client_test2("192.168.3.38", "1");
    return 0;
}

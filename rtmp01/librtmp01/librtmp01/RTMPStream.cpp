...
bool CRTMPStream::SendH264File(const char *pFileName)
{
    ...
    uint8_t *buffer = (uint8_t *)malloc(m_nFileBufSize);
    int aWidth = 0;
    int aHeight = 0;
    
    CH264Decoder *h264Decoder = new CH264Decoder();
//    h264Decoder->test();
    
    h264Decoder->initial();
//    h264Decoder->decode(m_pFileBuf,m_nFileBufSize,buffer,&aWidth,&aHeight);
    ...
    if (false) {
        ...
        unsigned char test1[1024] = {0x00};
        
        test1[3] = 0x01;
        
        int spslen = base64_decode(test1+4, "J2QAHqxWwLQ9kA==", strlen("J2QAHqxWwLQ9kA=="));
        
        test1[4+spslen+3] = 0x01;
        
        int ppslen = base64_decode(test1+4+spslen+4, "KO48sA==", strlen("KO48sA=="));
        {
            FILE *fp;
            
            fp = fopen("/Users/lbb/Desktop/day/rtmp/out.h264","wb");
            if(!fp)return -1;
            
            
            
            fwrite(test1,4+spslen+4+ppslen,1,fp);
            fwrite(m_pFileBuf,m_nFileBufSize,1,fp);
            fclose(fp);
        }
        
        h264Decoder->decode(test1,4+spslen+4+ppslen,buffer,&aWidth,&aHeight);
    } else {
        ...
        h264Decoder->decode(naluUnit.data-4,naluUnit.size+8,buffer,&aWidth,&aHeight);
    ...
    {
        h264Decoder->decode3(naluUnit.data-4,naluUnit.size+8,buffer,&aWidth,&aHeight);
...
bool CRTMPStream::ReadOneNaluFromBuf(NaluUnit &nalu)
{
    ...
    while(i<m_nFileBufSize)
    {
           ...
           )
        {
            ...
            printf("  ReadOneNaluFromBuf size:%d\n",nalu.size);

...
static int  sread(SOCKET s, void* buf, int len)
{
    return recv(s,(char *)buf,len,NULL);
}

static int swrite(SOCKET s, void* buf, int len)
{
    return send(s,(char *)buf,len,NULL);
}
...
static int wsSendPacket(WsHandler* ws, int opcode, unsigned char* dat, long len)
{
    ...
    *b++ = 0x80 + opcode;
    ...
    while (count < len)
    {
        int nrbytes = swrite(sock, dat + count, len - count);
        if (nrbytes < 0)
        {
...
static bool handleClient(void* ctx)
{
    WsHandler* ws = (WsHandler*)ctx;
    WsServer* srv = ws->server;
    char* buf = ws->buf;
...
static bool listenForClients(void* ctx)
{
    WsServer* obj = (WsServer*)ctx;
    obj->cont = TRUE;
    while (obj->cont)
    {
        ...
        //            return FALSE;
        //            }
        webrtc::ThreadWrapper* _thread = webrtc::ThreadWrapper::CreateThread(handleClient, ws,
            webrtc::kHighestPriority,
            "handleClient");
...
int wsServe(WsServer* obj)
{
    ...
    webrtc::ThreadWrapper* _thread = webrtc::ThreadWrapper::CreateThread(listenForClients, obj,
        webrtc::kHighestPriority,
        "listenForClients");
